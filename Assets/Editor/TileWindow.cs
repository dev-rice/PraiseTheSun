...
public class TileWindow : EditorWindow
{
	...
	void OnGUI()
	{
		...
		if (!Directory.Exists(Application.dataPath + "/Sprites/"))
		{
			...
			Debug.Log("Created Sprites Directory");
		}
		files = Directory.GetFiles(Application.dataPath + "/Sprites/", "*.png");
		options = new string[files.Length];
		EditorGUILayout.LabelField("Tile Map", GUILayout.Width(256));
		for(int i = 0; i < files.Length; i++)
		{
			options[i] = files[i].Replace(Application.dataPath + "/Sprites/", "");
		}
		...
		{
		allSprites = AssetDatabase.LoadAllAssetsAtPath("Assets/Sprites/" + options[index]).Select(x => x as Sprite).Where(x => x != null).ToArray();
